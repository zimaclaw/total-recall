...
# Skeleton конфиг
SKELETON_TAIL_PAIRS = settings.skeleton_tail_pairs
SKELETON_SUMMARY_MAX_PAIRS = settings.skeleton_summary_max_pairs
SKELETON_SUMMARY_MAX_TOKENS = settings.skeleton_summary_max_tokens
SKELETON_SUMMARY_ENABLED = settings.skeleton_summary_enabled
SKELETON_SUMMARY_CACHE = settings.skeleton_summary_cache
...
def cmd_get_skeleton(session_id: str):
    """
    ...
    4. Если M > SKELETON_TAIL_PAIRS:
       start = max(M - SKELETON_TAIL_PAIRS - SKELETON_SUMMARY_MAX_PAIRS, 0)
       end = M - SKELETON_TAIL_PAIRS
       summary_pairs = пары[start:end]
       tail_pairs = пары[end:]
    5. Проверить кэш по ключу "{session_id}:{start}:{end}"
    6. Если кэш есть → взять summary из кэша
       Если кэша нет → вызвать LLM для summary
    7. Сохранить summary в skeleton_cache
    8. Вернуть: summary + tail_pairs дословно
    """
    with get_conn() as conn:
        with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
            ...
            cur.execute("""
                ...
                WHERE session_id = %s AND pair_id IS NOT NULL
                GROUP BY pair_id
                ORDER BY pair_ts ASC
            """, (session_id,))
            pairs = cur.fetchall()
            
            M = len(pairs)
            
            # Если пар <= SKELETON_TAIL_PAIRS — возвращаем все пары дословно
            if M <= SKELETON_TAIL_PAIRS:
                tail_pairs = pairs
                summary = None
            else:
                # Определяем range для summary
                end = M - SKELETON_TAIL_PAIRS
                start = max(end - SKELETON_SUMMARY_MAX_PAIRS, 0)
                
                summary_pairs = pairs[start:end]
                tail_pairs = pairs[end:]
                ...
                elif SKELETON_SUMMARY_ENABLED and summary_pairs:
                    # Формируем промпт для LLM
                    pairs_text = []
                    for pair_id in [p['pair_id'] for p in summary_pairs]:
                        cur.execute("""
                            SELECT role, content
                            FROM session_messages
                            WHERE session_id = %s AND pair_id = %s
...
{pairs_text}"""
            ...
            for pair in tail_pairs:
                ...
                pair_msgs = cur.fetchall()
                for msg in pair_msgs:
                    ts_str = msg['ts'].strftime('%H:%M')
                    tail_lines.append(f"[{msg['role'].upper()} {ts_str}] {msg['content']}")
            
            tail_text = "\n".join(tail_lines) if tail_lines else ""
            
            # Сохраняем в кэш если сгенерировали summary
            if summary and SKELETON_SUMMARY_CACHE and not cached:
                conn.commit()
...
def main():
    ...
    elif args.cmd == "get_skeleton":
        cmd_get_skeleton(args.session_id)
    elif args.cmd == "focus":
